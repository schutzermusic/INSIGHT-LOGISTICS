/**
 * Complete Brazilian Cities Database — All 5,571 municipalities
 * Source: IBGE (Instituto Brasileiro de Geografia e Estatística)
 * Format: "City Name - UF"
 */

const BRAZILIAN_CITIES_ALL = [
    'Abadia de Goiás - GO',
...
/**
 * Get all Brazilian cities sorted alphabetically
 */
export function getAllBrazilianCities() {
    return BRAZILIAN_CITIES_ALL;
}

/**
 * Search Brazilian cities by partial name or state abbreviation
 * @param {string} query - Search term (min 2 chars)
 * @returns {string[]} Matching city names
 */
export function searchAllBrazilianCities(query) {
    if (!query || query.length < 2) return [];
    const q = query.toLowerCase();
    const isUF = q.length === 2;
    return BRAZILIAN_CITIES_ALL.filter(c => {
        const lower = c.toLowerCase();
        if (isUF) {
            // Match state abbreviation exactly
...
export default BRAZILIAN_CITIES_ALL;
